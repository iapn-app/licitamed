import { AlertTriangle, TrendingDown, TrendingUp, Package, Truck, Shield } from "lucide-react";
import { NeonCard } from "@/components/shared/neon-card";
import type { AlertaPreco } from "@/types/preco";
...
export function PriceAlerts({ alertas }: PriceAlertsProps) {
  if (alertas.length === 0) return null;

  return (
    <NeonCard className="overflow-hidden">
      <div className="px-5 py-4 border-b border-neutral-100">
        <h3 className="text-sm font-semibold text-neutral-900">Alertas de precificação</h3>
        <p className="text-xs text-neutral-400 mt-0.5">{alertas.length} alerta{alertas.length !== 1 ? "s" : ""} identificado{alertas.length !== 1 ? "s" : ""}</p>
      </div>
          ...
          return (
            <div key={idx} className={cn("flex items-start gap-3 px-5 py-3.5", cfg.bg)}>
              ...
              <div className="flex-1 min-w-0">
                ...
                {alerta.item && (
                  <p className="text-xs text-neutral-500 mt-0.5">Item: {alerta.item}</p>
                )}
              </div>
